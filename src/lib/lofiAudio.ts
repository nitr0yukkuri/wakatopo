...
export const getLofiBgmProfile = (
  weather: WeatherType,
  activity: number,
  ...
  switch (activeWorkId) {
    case "01":
      // Raised one octave (196–294 Hz) so the notes are audible on laptop/phone speakers.
      return {
        notes: [196.0, 220.0, 293.66, 261.63],
        highRatio: 1.28,
        pulseRatio: 1.14,
        waveform: "sine",
        accentWaveform: "sine",
  ...
  // Home-screen default profiles.
  // Notes are in the 155–330 Hz range (one octave above the old bass-only range of 78–165 Hz).
  // Laptop and phone speakers reproduce below ~150 Hz very poorly, so the old sub-bass
  // notes were effectively silent on the most common hardware.
  // The deepBed voice (base × 0.5) drops to 78–165 Hz which still gives warmth on
  // good speakers without depending on sub-bass reproduction.
  // tickMs is also shortened to make the ambient loop feel more ‘flowing’ rather than
  // a long silence between bursts.
  switch (weather) {
    case "Night":
      return {
