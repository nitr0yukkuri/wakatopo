...
const copyByLang = {
  ja: {
    ...
    conceptLeadBefore: 'Webサイトを「静的な情報の羅列」から ',
    conceptLeadHighlight: '「呼吸する惑星」',
    conceptLeadAfter: 'へ。',
    conceptDesc: 'GitHubの活動が地形を作り、現実の天気が空気を変える。エンジニアとしての生存記録を有機的なデジタルアートへ昇華させる実験。',
    ...
    <main className="relative w-full min-h-[100dvh] text-white font-sans bg-[#050505]">
      ...
      <div className="fixed inset-0 z-50 pointer-events-none p-6 md:p-8 flex flex-col justify-between text-xs font-mono tracking-widest text-gray-500">
        ...
        <div className="absolute top-6 right-6 md:top-8 md:right-8 text-right flex flex-col gap-1 pointer-events-auto">
          ...
          <span className="hidden sm:inline">{t.target}</span>
        </div>
