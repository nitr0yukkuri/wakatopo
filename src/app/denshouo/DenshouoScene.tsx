...
function useWebGLSupport() {
    ...
    useEffect(() => {
        // WebGL support is an external browser capability; read it after mount.
...
const OCEAN_BUBBLE_DATA = (() => {
    ...
    for (let index = 0; index < count; index++) {
        ...
        positions[index * 3 + 2] = -Math.random() * 12 - 1.2;
        scales[index] = Math.random() * 2.0 + 0.8;
        speeds[index] = Math.random() * 0.9 + 0.5;
...
function OceanSurface() {
    ...
    useFrame((state, delta) => {
        // R3F uniforms are mutable GPU inputs, not React state.
...
function OceanBubbles() {
    ...
    const { positions, scales, speeds, seeds } = OCEAN_BUBBLE_DATA;
